...
class ExcelWriter:
    ...
    def write_VRESProfiles(data: pd.DataFrame, file_path: str):
        ...
        # Get file paths
        file_path = ExcelWriter.__checkFilepath(file_path)
        ...
        for i, column in enumerate(pivot_df.columns):
            ...
            ExcelWriter.__copyCellStyle(colTemplate_readableName, targetSheet.cell(row=3, column=i + 7))
            targetSheet.cell(row=4, column=i + 7, value=column)
            ExcelWriter.__copyCellStyle(colTemplate_dbName, targetSheet.cell(row=4, column=i + 7))
            ExcelWriter.__copyCellStyle(colTemplate_description, targetSheet.cell(row=5, column=i + 7))
            ExcelWriter.__copyCellStyle(colTemplate_dbBehavior, targetSheet.cell(row=6, column=i + 7))

            targetSheet.cell(row=7, column=i + 7, value=colTemplate_unit.value)
            ExcelWriter.__copyCellStyle(colTemplate_unit, targetSheet.cell(row=7, column=i + 7))
            ...
            for j, cell in enumerate(row):
                ...
                match cell.column:
                    case 1:
                        continue
                    case 2:
                        ExcelWriter.__copyCellStyle(rowTemplate_id, cell)
                    case 3:
                        ExcelWriter.__copyCellStyle(rowTemplate_rp, cell)
                    case 4:
                        ExcelWriter.__copyCellStyle(rowTemplate_g, cell)
                    case 5:
                        ExcelWriter.__copyCellStyle(rowTemplate_dataPackage, cell)
                    case 6:
                        ExcelWriter.__copyCellStyle(rowTemplate_dataSource, cell)
                    case _:
                        ExcelWriter.__copyCellStyle(rowTemplate_values, cell)
...
if __name__ == "__main__":
    ...
    ExcelWriter.write_VRESProfiles(data, "examples/output/Power_VRESProfiles_output.xlsx")  # Write data to Excel file
    pass
